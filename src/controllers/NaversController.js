...
class NaversController {
  ...
  async show(req, res) {
    ...
    const projects = await knex('projects')
      .join(
        ...
        'projects.id',
        '=',
        'naver_project_relation.project_id',
      )
  ...
  async store(req, res) {
    ...
    try {
      ...
      return res.status(400).json({
        message:
          'One of the included projects still does not exist. Please create it in the apropriate route with an empty navers array and then comeback with the right id.',
      });
    }
  }
}
